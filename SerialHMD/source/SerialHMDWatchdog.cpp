...
// *******************************************************
// Static Watchdog Thread Function - frequently checking if HMD has been connected
// *******************************************************
	...
	while (!g_watchdogExiting && numResets < 5)
	{
		...
		{ // Set identification request to RPi
			...
			{ // Read response and check for identification code
				...
				char response[128];
				error = serRPiPort->readSerialPort(response, &responseSize);
				...
				{ // Identified as Serial HMD
					...
					goto idle; // Success, stop watchdog
				}
				else if (failedResponses >= 10)
			...
			error = serRPiPort->writeSerialPort((char *)("#R#"), 3);
			error = serRPiPort->writeSerialPort((char *)("#I#"), 3);
			if (error != 0)
				DriverLog("Serial Port Write Error: %d \n", error);
...
// *******************************************************
// SerialHMDWatchdog - Watchdog Class Implementation
// *******************************************************
